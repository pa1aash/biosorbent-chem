...
## 2026-08-15 — Computational arm: all seventeen jobs analysed; the ordering does not hold

Session S07. Analysis only — no job was re-run, no structure re-optimised, no number adjusted.
Full account in [`../dft/analysis/RESULTS_SUMMARY.md`](../dft/analysis/RESULTS_SUMMARY.md); the
free-energy composition is specified in
[`../dft/analysis/G_COMPOSITION.md`](../dft/analysis/G_COMPOSITION.md).

### The result

**The computed ordering does not reproduce Pb > Cu > Zn at any protonation state.** Lead is
preferred over copper only at P0 (ΔΔG = −17.8 kJ/mol); at P1 and P2 **copper is preferred over
lead** by +11.2 and +10.0 kJ/mol. All three ΔΔG(Pb−Cu) values are denticity-matched, so none can be
set aside on comparability grounds. Lead is preferred over zinc at all three states. The ordering is
**Pb > Zn > Cu at P0** and **Cu > Pb > Zn at P1 and P2** — it inverts on deprotonation.

`DFT_PROTOCOL.md` §1.3 pre-committed to this outcome: *"The reported claim is the one that survives
all three protonation states… If it does not, the report reports that instead."* **Only Pb > Zn
survives all three.**
...
| Date | Decision | Rationale |
|---|---|---|
| 2026-08-15 | **`thermo.py` consumes ORCA's own `G-E(el)` rather than re-deriving the thermal correction.** | ORCA 6.1.1 printed `Quasi RRHO ... True` and *"QRRHO of S. Grimme … using a reference frequency of 100.0 cm-1"* on all seventeen jobs. That is exactly what protocol §3.4 specifies. **This supersedes the S04 ruling** that quasi-RRHO would be applied in post-processing — that ruling's premise, that ORCA defaults to raw RRHO, was wrong. Re-deriving would either downgrade the treatment or reproduce ORCA's arithmetic less reliably. The frequency list is still parsed so the low-mode population is visible rather than assumed: **16 of 17 species have their lowest real mode below 100 cm⁻¹**, the lowest being 14.75 cm⁻¹. |
| 2026-08-15 | **x = 2 and Δn = +1 are reported as FIXED by the computed cluster compositions, not as measured quantities.** | `[M(H₂O)₆]²⁺` holds six waters and `[M(L)(H₂O)₄]^q` holds four, so two are released, in all nine. **No `[M(L)(H₂O)₅]^q` species was ever built or run**, so no x = 1 reaction can be evaluated from this job set. What *is* measured is denticity and the product's first-shell coordination number. This is **more favourable than S06 assumed**: identical stoichiometry across all nine means ΔG_ss and the released-water terms cancel exactly in every ΔΔG. |
| 2026-08-15 | **Denticity verdicts use the element-specific first-shell cutoffs (Pb 3.20 Å, Cu/Zn 2.80 Å) and the cutoff sensitivity is reported alongside.** | §3.7 requires production and pre-screen verdicts to be directly comparable, and the pre-screen used element-specific cutoffs. **One verdict is sensitive**: `pb_P0_cplx`'s single galloyl contact at 2.936 Å reads MONODENTATE at 3.20 Å and DISSOCIATED at a uniform 2.80 Å. It is bidentate under neither. Reported rather than left implicit, because it is what a computational referee will test. |
| 2026-08-15 | **The Stage-1 outline's α values are NOT written into `CANONICAL_NUMBERS.yaml`.** `ddg_pb_cu_exp_kJ_per_mol` and `ddg_pb_zn_exp_kJ_per_mol` stay `PENDING`. | YAML rule 3 and CLAUDE.md §3: the outline's numbers are claims to be tested, not inputs. The −RT ln α arithmetic is reported in `RESULTS_SUMMARY.md` §5 for the §4.6 comparison; the keys fill when α is derived from the AAS data. |
| 2026-08-15 | **The Pb–O bond-length validation deviation is NOT computed.** | §6 requires comparison against a published EXAFS / X-ray hydration structure. Per CLAUDE.md §6 no literature value is quoted from recollection. The computed means (2.537 Å over 5 first-shell donors for `pb_aquo6`, 2.604 Å over 6 for `pb_aquo8`) stand; the deviation waits on a verified citation. Carried to `DATA_REQUEST.md`. |
| 2026-08-15 | **Per-metal DFT keys without a protonation-state suffix were removed from `CANONICAL_NUMBERS.yaml` and replaced by `_P0` / `_P1` / `_P2` keys.** | They are ill-defined: ΔG_exchange for lead runs from −9.5 kJ/mol at P0 to −190.5 kJ/mol at P2, so a single `dg_exchange_pb_kJ_per_mol` could only ever be wrong. 129 keys now `VERIFIED`, each naming the ORCA output file it came from. Generated by `dft/analysis/emit_canonical.py`; nothing hand-typed. |

### Attack register

**A02 residual discharged** — ⟨S²⟩ = 0.751411 / 0.751823 / 0.751649 / 0.752207 across the four Cu
species, +0.19% to +0.29% from the ideal 0.750, all far below the 0.80 flagging threshold.
**A03 gains second, independent machine evidence** — the five `*_atom82.out` auxiliary atomic
reference calculations print `Group 1, Type Pb ECP (replacing 60 core electrons, lmax=3)`.
**A31 RESOLVED**, three-state picture complete. **A33 ARMOURED**, was ACCEPTED RISK.
**A05 worsened to a sign disagreement. A32 raised 🟡 → 🟠. A34 added, 🔴 Critical.**

### Findings requiring Palaash's ruling

| Ref | Finding |
|---|---|
| **D-09** | **`DFT_PROTOCOL.md` §2.2 is out of date and understates the project's position.** It concedes the GFN2 CN = 8 preference as an open limitation. That preference was tested at the production level of theory in solution and **does not survive: ΔG = +51.2 kJ/mol in favour of CN = 6**, a 127.0 kJ/mol swing from GFN2, driven entirely by the entropy of binding two extra waters. The eight-water cluster also relaxes to a six-coordinate first shell. §2.2 must be rewritten. **A real, different limitation replaces it**: neither lead aquo ion holds all its waters in the inner sphere — `pb_aquo6` expels one to 4.005 Å, giving a five-coordinate first shell. |
| **D-10** | The S06 log entry above records "both x = 1, Δn = 0" for Pb and Cu at P0. **That describes a reaction for which no calculation exists.** Per this log's convention the entry is left standing and corrected here rather than edited. |
| **D-11** | **Attack A05's pre-written armour no longer answers the attack.** It assumes a magnitude discrepancy in the right direction; two of six comparisons **disagree in sign**. §4.6.3 must be rewritten before it can be used. |
| **D-12** | **The protonation state now determines which metal is preferred.** Attack **A32** (unsourced pK_a) is therefore load-bearing: it selects between two contradictory answers. §1.3's argument that the three-state design makes the pK_a unnecessary **no longer holds**, and the report cannot argue from the P0 result without a verified pK_a. |
| **D-13** | **The report's central claim is not supported by the computational arm at the deprotonated states.** §4.7, the abstract and the title's framing need restructuring around what was found. What the calculation robustly supports: Pb > Zn at every state; lead hemidirected and the comparators not; and lead's hemidirection **intrinsic to the aqueous ion rather than induced by the galloyl pocket**. Tracked as attack **A34**. |
| — | **"D-08", cited in the S07 session brief as the ruling that x is measured rather than assumed, does not exist in this log**, which runs D-01 to D-07. Its substance was followed. If the ruling was made, it should be recorded. |

### Tooling added

`dft/analysis/` — `orca_parse.py` (single-source ORCA 6.1.1 parser; cclib 1.8.1 still cannot read
these files), `integrity_gate.py`, `verify_c01.py`, `denticity.py`, `thermo.py`, `selectivity.py`,
`run_hemidirection.py`, `pb_coordination.py`, `emit_canonical.py`, plus `G_COMPOSITION.md` and
`RESULTS_SUMMARY.md`. Every script exits non-zero on a failed gate.
